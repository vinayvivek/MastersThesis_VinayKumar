...
def train_epoch(run_id, learning_rate2,  epoch, data_loader, model, criterion, optimizer, writer, use_cuda):
    ...
    for param_group in optimizer.param_groups:
        ...
        save_payload = {
            "text": "Learning rate is: {}".format(param_group['lr'])
        }
        send_slack_message(save_payload, slack_webhook)
    ...
    for i, (inputs, label, vid_path) in enumerate(data_loader):
        ...
        if i % 24 == 0: 
            ...
            save_payload = {
                "text": f'Training Epoch {epoch}, Batch {i}, Loss: {np.mean(losses) :.5f}'
            }
            send_slack_message(save_payload, slack_webhook)
    ...
    save_payload = {
        "text": 'Training Epoch: %d, Loss: %.4f' % (epoch,  np.mean(losses))
    }
    send_slack_message(save_payload, slack_webhook)
...
def val_epoch(run_id, epoch,mode,pred_dict,label_dict, data_loader, model, criterion, writer, use_cuda):
    ...
    save_payload = {
        "text": f'validation at epoch {epoch} - mode {mode} '
    }
    send_slack_message(save_payload, slack_webhook)
    ...
    for i, (inputs, label, vid_path, _) in enumerate(data_loader):
        ...
        if len(inputs.shape) != 1:
            ...
            if i+1 % 45 == 0:
                ...
                save_payload = {
                    "text": f'Validation Epoch, {epoch}, mode, {mode}, Batch, {i}, - Loss : , {np.mean(losses)}'
                }
                send_slack_message(save_payload, slack_webhook)
    ...
    save_payload = {
        "text": f'Epoch {epoch}, mode {mode}, Accuracy: {accuracy*100 :.3f}'
    }
    send_slack_message(save_payload, slack_webhook)
...
def train_classifier(run_id, restart, saved_model, linear):
    ...
    if restart:
        ...
        if os.path.exists(saved_model_file):
            ...
            save_payload = {
                "text": f'No such model exists: {saved_model_file} :('
            }
            send_slack_message(save_payload, slack_webhook)
            ...
            if not (saved_model == None or len(saved_model) == 0 or saved_model =="kin400"):
                ...
                save_payload = {
                    "text": f'Trying to load {saved_model}'
                }
                send_slack_message(save_payload, slack_webhook)
                ...
                save_payload = {
                    "text": f'It`s a baseline experiment!'
                }
                send_slack_message(save_payload, slack_webhook)
    ...
    save_payload = {
        "text": f'Train dataset length: {len(train_dataset)}'
    }
    send_slack_message(save_payload, slack_webhook)
    ...
    save_payload = {
        "text": f'Train dataset steps per epoch: {len(train_dataset)/params.batch_size}'
    }
    send_slack_message(save_payload, slack_webhook)
    ...
    for epoch in range(epoch0, params.num_epochs):
        ...
        try:
            ...
            if scheduler_epoch == params.scheduler_patience:
                ...
                save_payload = {
                    "text": f'Dropping learning rate to {learning_rate2/10} for epoch'
                }
                send_slack_message(save_payload, slack_webhook)
            ...
            if epoch in val_array:
                ...
                for val_iter in range(len(modes)):
                    try:
                        ...
                        if val_iter ==0:
                            ...
                            save_payload = {
                                "text": f'Validation dataset length: {len(validation_dataset)}'
                            }
                            send_slack_message(save_payload, slack_webhook)
                            ...
                            save_payload = {
                                "text": f'Validation dataset steps per epoch: {len(validation_dataset)/params.v_batch_size}'
                            }
                            send_slack_message(save_payload, slack_webhook)
                        ...
                        save_payload = {
                            "text": f'Running Avg Accuracy is for epoch {epoch}, mode {modes[val_iter]}, is {accuracy11*100 :.3f}% '
                        }
                        send_slack_message(save_payload, slack_webhook)
                        ...
                        save_payload = {
                            "text": f'Failed epoch {epoch}, mode {modes[val_iter]}, is {accuracy11*100 :.3f}% '
                        }
                        send_slack_message(save_payload, slack_webhook)
                ...
                save_payload = {
                    "text": f'Val loss for epoch {epoch} is {np.mean(val_losses)} \n'
                            f'Correct Count is {correct_count} out of {len(c_pred)} \n'
                            f'Overall Accuracy is for epoch {epoch} is {accuracy1 * 100 :.3f}% '
                }
                send_slack_message(save_payload, slack_webhook)
            ...
            if accuracy > best_acc:
                ...
                save_payload = {
                    "text": f'Epoch {epoch} is the best model till now for {run_id}!'
                }
                send_slack_message(save_payload, slack_webhook)
        ...
        save_payload = {
            "text": f'Time taken for Epoch-{epoch} is {taken}'
        }
        send_slack_message(save_payload, slack_webhook)
        ...
        save_payload = {
            "text": f'Train dataset length: {len(train_dataset)} \n'
                    f'Train dataset steps per epoch: {len(train_dataset) / params.batch_size}'
        }
        send_slack_message(save_payload, slack_webhook)
        ...
        if learning_rate2 < 1e-5:
            ...
            save_payload = {
                "text": f'Learning rate is very low now, ending the process...s'
            }
            send_slack_message(save_payload, slack_webhook)
