...
class baseline_dataloader_train_strong(Dataset):
    ...
    def build_clip(self, vid_path):
        ...
        try:
            ...
            frames_full = start_frame_full + np.asarray(
                    ...
                    if (count in frames_full):
                        full_clip.append(
                            self.augmentation(frame, random_array[0], x_erase[0], y_erase[0], cropping_factor1[0], \
                                              x0, y0, contrast_factor1[0], hue_factor1[0], saturation_factor1[0],
...
class multi_baseline_dataloader_val_strong(Dataset):
    ...
    def __init__(self, shuffle=True, data_percentage=1.0, mode=0, \
                 hflip=0, cropping_factor=0.8, split=4):
        if split == 1:
            self.all_paths = open(os.path.join(cfg.path_folder, 'ucfTrainTestlist/testlist01.txt'),
                                  'r').read().splitlines()
        elif split == 2:
            self.all_paths = open(os.path.join(cfg.path_folder, 'ucfTrainTestlist/testlist02.txt'),
                                  'r').read().splitlines()
        elif split == 3:
            self.all_paths = open(os.path.join(cfg.path_folder, 'ucfTrainTestlist/testlist03.txt'),
                                  'r').read().splitlines()
        elif split == 4:
            self.all_paths = open(os.path.join(cfg.path_folder, 'ucfTrainTestlist/testlist03_updated.txt'),
                                  'r').read().splitlines()
        else:
            print(f'Invalid split input: {split}')
    ...
    def augmentation(self, image):
        ...
        if self.cropping_factor <= 1:
            ...
            int(params.ori_reso_h * self.cropping_factor), int(params.ori_reso_h * self.cropping_factor)))
