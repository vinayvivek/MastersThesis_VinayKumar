...
slack_webhook = "https://hooks.slack.com/services/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"
...
# send_slack_message(save_payload, slack_webhook)



def train_epoch(scaler, run_id, learning_rate2, epoch, criterion, data_loader, model, optimizer, writer, use_cuda,
                criterion2):
    print('train at epoch {}'.format(epoch))

    save_payload = {
        "text": 'train at epoch {}'.format(epoch)
    }
    send_slack_message(save_payload, slack_webhook)
    ...
    for i, (sparse_clip, dense_clip0, dense_clip1, dense_clip2, dense_clip3, a_sparse_clip, \
        ...
        sparse_clip = sparse_clip.permute(0, 2, 1, 3,
                                          4)  # aug_DL output is [120, 16, 3, 112, 112]], model expects [8, 3, 16, 112, 112]
        ...
        a_sparse_clip = a_sparse_clip.permute(0, 2, 1, 3,
                                              4)  # aug_DL output is [120, 16, 3, 112, 112]], model expects [8, 3, 16, 112, 112]
        ...
        with autocast():
            ...
            criterion = NTXentLoss(device='cuda', batch_size=out_sparse[0][0].shape[0], temperature=params.temperature,
                                   use_cosine_similarity=False).cuda()
            criterion_local_local = NTXentLoss(device='cuda', batch_size=4, temperature=params.temperature,
                                               use_cosine_similarity=False).cuda()
            ...
            for ii in range(out_dense[0][0].shape[0]):  # for loop in the batch size
                loss_local_local += criterion_local_local(torch.stack(out_dense[0], dim=1)[ii],
                                                          torch.stack(out_dense[1], dim=1)[ii])

            loss_global_local = 0
            for ii in range(2):
                for jj in range(2):
                    loss_global_local += criterion2(torch.stack(out_sparse[ii][1:], dim=1),
                                                    torch.stack(out_dense[jj], dim=1), params.temperature)
        ...
        if (i + 1) % 100 == 0:
            ...
            payload_batch = {
                "text": f'Training Epoch {epoch}, Batch {i}, Loss: {np.mean(losses) :.5f}, losses_local_local: {np.mean(losses_local_local) :.5f}, losses_global_local: {np.mean(losses_global_local) :.5f}, losses_ic1: {np.mean(losses_ic1) :.5f}, losses_ic2: {np.mean(losses_ic2) :.5f}'}
            send_slack_message(payload_batch, slack_webhook)
    ...
    payload_epoch = {
        "text": f'Training Epoch: {epoch}, Loss: {np.mean(losses)}, Training Loss: {np.mean(losses)}, losses_local_local: {np.mean(losses_local_local)}, losses_global_local: {np.mean(losses_global_local)},losses_ic2: {np.mean(losses_ic2)}, losses_ic1: {np.mean(losses_ic1)}'
              }
    send_slack_message(payload_epoch, slack_webhook)
...
#     if worker_id == problematic_worker_id:
#         sys.exit(0)
...
def train_classifier(run_id, restart):
    ...
    if restart:
        ...
        saved_model_file = "/home/multi-sy-20/PycharmProjects/TCLR/tclr_pretraining/ss_saved_models/SS_CL_VC_AR_EXP_1_Ep500/model_temp.pth"
    ...
    criterion = NTXentLoss(device='cuda', batch_size=params.batch_size, temperature=params.temperature,
    ...
    for epoch in range(epoch0, params.num_epochs):
        ...
        save_epoch_payload = {
            "text": f'Epoch {epoch} started'
        }
        send_slack_message(save_epoch_payload, slack_webhook)
        ...
        start = time.time()
        try:
            model, loss, scaler = train_epoch(scaler, run_id, learning_rate2, epoch, criterion, train_dataloader, model,
                                              optimizer, writer, use_cuda, criterion2=global_local_temporal_contrastive)
            ...
            if loss < best_score:
                ...
                scheduler_epoch = 0
                save_payload = {
                    "text": f'improved model saved at {save_file_path}'
                }
                send_slack_message(save_payload, slack_webhook)
            elif epoch % 5 == 0:
                save_dir = os.path.join(cfg.saved_models_dir, run_id)
                ...
                torch.save(states, save_file_path)
                save_epoch_payload = {
                    "text": f'modal at {epoch} saved at {save_file_path}'
                }
                send_slack_message(save_epoch_payload, slack_webhook)
            if loss > best_score:
                scheduler_epoch += 1
            ...
            send_slack_message(payload_error, slack_webhook)
            print('-' * 60)
            continue
            # pass
        train_dataset = ss_dataset_gen1(shuffle=True, data_percentage=params.data_percentage)
