...
class ss_dataset_gen1(Dataset):
    ...
    def __init__(self, shuffle=True, data_percentage=1.0, split=4):

        # self.all_paths = open(os.path.join(cfg.path_folder,'train_vids.txt'),'r').read().splitlines()
        if split == 1:
            self.all_paths = open(os.path.join(cfg.path_folder, 'ucfTrainTestlist/trainlist01.txt'),
                                  'r').read().splitlines()
        elif split == 2:
            self.all_paths = open(os.path.join(cfg.path_folder, 'ucfTrainTestlist/trainlist02.txt'),
                                  'r').read().splitlines()
        elif split == 3:
            self.all_paths = open(os.path.join(cfg.path_folder, 'ucfTrainTestlist/trainlist03.txt'),
                                  'r').read().splitlines()

        elif split == 4:
            self.all_paths = open(os.path.join(cfg.path_folder, 'ucfTrainTestlist/trainlist03_updated.txt'),
                                  'r').read().splitlines()
        else:
    ...
    def __getitem__(self, index):
        sparse_clip, dense_clip0, dense_clip1, dense_clip2, dense_clip3, \
            a_sparse_clip, a_dense_clip0, a_dense_clip1, a_dense_clip2, a_dense_clip3, list_sparse, list_dense, vid_path = self.process_data(
            index)
    ...
    def process_data(self, idx):
        ...
        sparse_clip, dense_clip0, dense_clip1, dense_clip2, dense_clip3, \
            a_sparse_clip, a_dense_clip0, a_dense_clip1, a_dense_clip2, a_dense_clip3, list_sparse, list_dense = self.build_clip(
            vid_path)
        return sparse_clip, dense_clip0, dense_clip1, dense_clip2, dense_clip3, \
            a_sparse_clip, a_dense_clip0, a_dense_clip1, a_dense_clip2, a_dense_clip3, list_sparse, list_dense, vid_path
    ...
    def build_clip(self, vid_path):
        ...
        try:
            ...
            frames_dense = [
                [frames_sparse[j * 4]] + [frames_sparse[j * 4] + i * sr_dense for i in range(1, params.num_frames)] for
                j in range(4)]
            ...
            cropping_factor1 = np.random.uniform(0.6, 1, size=(
            10,))  # on an average cropping factor is 80% i.e. covers 64% area
            x0 = [np.random.randint(0, params.ori_reso_w - params.ori_reso_w * cropping_factor1[ii] + 1) for ii in
                  range(10)]
            y0 = [np.random.randint(0, params.ori_reso_h - params.ori_reso_h * cropping_factor1[ii] + 1) for ii in
                  range(10)]
            ...
            while (cap.isOpened()):
                ...
                if ret == True:
                    if (count in frames_sparse):
                        sparse_clip.append(
                            self.augmentation(frame, random_array[0], x_erase[0], y_erase[0], cropping_factor1[0], \
                                              x0[0], y0[0], contrast_factor1[0], hue_factor1[0], saturation_factor1[0],
                                              brightness_factor1[0], \
                                              gamma1[0], erase_size1[0], erase_size2[0], random_color_dropped[0]))
                        a_sparse_clip.append(
                            self.augmentation(frame, random_array[1], x_erase[1], y_erase[1], cropping_factor1[1], \
                                              x0[1], y0[1], contrast_factor1[1], hue_factor1[1], saturation_factor1[1],
                                              brightness_factor1[1], \
                                              gamma1[1], erase_size1[1], erase_size2[1], random_color_dropped[1]))
                        list_sparse.append(count)
                    if (count in frames_dense[0]):
                        dense_clip0.append(
                            self.augmentation(frame, random_array[2], x_erase[2], y_erase[2], cropping_factor1[2], \
                                              x0[2], y0[2], contrast_factor1[2], hue_factor1[2], saturation_factor1[2],
                                              brightness_factor1[2], \
                                              gamma1[2], erase_size1[2], erase_size2[2], random_color_dropped[2]))
                        a_dense_clip0.append(
                            self.augmentation(frame, random_array[3], x_erase[3], y_erase[3], cropping_factor1[3], \
                                              x0[3], y0[3], contrast_factor1[3], hue_factor1[3], saturation_factor1[3],
                                              brightness_factor1[3], \
                                              gamma1[3], erase_size1[3], erase_size2[3], random_color_dropped[3]))
                        list_dense[0].append(count)
                    if (count in frames_dense[1]):
                        dense_clip1.append(
                            self.augmentation(frame, random_array[4], x_erase[4], y_erase[4], cropping_factor1[4], \
                                              x0[4], y0[4], contrast_factor1[4], hue_factor1[4], saturation_factor1[4],
                                              brightness_factor1[4], \
                                              gamma1[4], erase_size1[4], erase_size2[4], random_color_dropped[4]))
                        a_dense_clip1.append(
                            self.augmentation(frame, random_array[5], x_erase[5], y_erase[5], cropping_factor1[5], \
                                              x0[5], y0[5], contrast_factor1[5], hue_factor1[5], saturation_factor1[5],
                                              brightness_factor1[5], \
                                              gamma1[5], erase_size1[5], erase_size2[5], random_color_dropped[5]))
                        list_dense[1].append(count)
                    if (count in frames_dense[2]):
                        dense_clip2.append(
                            self.augmentation(frame, random_array[6], x_erase[6], y_erase[6], cropping_factor1[6], \
                                              x0[6], y0[6], contrast_factor1[6], hue_factor1[6], saturation_factor1[6],
                                              brightness_factor1[6], \
                                              gamma1[6], erase_size1[6], erase_size2[6], random_color_dropped[6]))
                        a_dense_clip2.append(
                            self.augmentation(frame, random_array[7], x_erase[7], y_erase[7], cropping_factor1[7], \
                                              x0[7], y0[7], contrast_factor1[7], hue_factor1[7], saturation_factor1[7],
                                              brightness_factor1[7], \
                                              gamma1[7], erase_size1[7], erase_size2[7], random_color_dropped[7]))
                        list_dense[2].append(count)
                    if (count in frames_dense[3]):
                        dense_clip3.append(
                            self.augmentation(frame, random_array[8], x_erase[8], y_erase[8], cropping_factor1[8], \
                                              x0[8], y0[8], contrast_factor1[8], hue_factor1[8], saturation_factor1[8],
                                              brightness_factor1[8], \
                                              gamma1[8], erase_size1[8], erase_size2[8], random_color_dropped[8]))
                        a_dense_clip3.append(
                            self.augmentation(frame, random_array[9], x_erase[9], y_erase[9], cropping_factor1[9], \
                                              x0[9], y0[9], contrast_factor1[9], hue_factor1[9], saturation_factor1[9],
                                              brightness_factor1[9], \
    ...
    def augmentation(self, image, random_array, x_erase, y_erase, cropping_factor1, \
        ...
        image = trans.functional.resized_crop(image, y0, x0, int(params.ori_reso_h * cropping_factor1),
                                              int(params.ori_reso_h * cropping_factor1), (params.reso_h, params.reso_w),
                                              interpolation=2)
...
def collate_fn2(batch):
    ...
    for item in batch:
        if not (None in item):
            ...
            # print('*******************************************************************')
            # print('item[11]', item[11], len(item[11]))
            # print('*******************************************************************')
            # list_dense.append(np.asarray(item[11]))
            # print("-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=")
            # print("list_sparse: ", list_sparse, "Length of list_sparse is :", len(list_sparse))
            # print("list_dense: ", list_dense, "Length of list_dense is :", len(list_dense))
            # # print("list_dense: ", list_dense)
            # print("-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=")
            # merged_list = sum([sublist for sublist in item[11]], [])
            # list_dense.append([merged_list])
            merged_list = np.concatenate([sublist for sublist in item[11]], axis=0)
            list_dense = np.asarray(merged_list)
...
#     for item in batch:
#         # Check if the item contains any None values
#         if not any(val is None for val in item):
#             # Convert sparse and dense clips to tensors and append them to the respective lists
#             sparse_clip.append(torch.stack(item[0], dim=0))
#             dense_clip0.append(torch.stack(item[1], dim=0))
#             dense_clip1.append(torch.stack(item[2], dim=0))
#             dense_clip2.append(torch.stack(item[3], dim=0))
...
#             a_dense_clip3.append(torch.stack(item[9], dim=0))
#
#             # Convert list_sparse and list_dense to NumPy arrays and append them to the respective lists
#             list_sparse.append(np.asarray(item[10]))
#
#             # Convert all elements of list_dense to a single NumPy array and append it to the list
#             merged_list = sum([np.asarray(element) for element in item[11]], [])
#             list_dense.append(merged_list)
#
#             # Append the video path to the vid_path list
#             vid_path.append(item[12])
#
#     # Check if there are any items in the lists
#     if sparse_clip:
#         # Stack the lists into tensors and return the results
#         sparse_clip = torch.stack(sparse_clip, dim=0)
#         dense_clip0 = torch.stack(dense_clip0, dim=0)
#         dense_clip1 = torch.stack(dense_clip1, dim=0)
#         dense_clip2 = torch.stack(dense_clip2, dim=0)
...
#             list_sparse, list_dense, vid_path
#     else:
#         # Return empty tensors and lists if there are no items
